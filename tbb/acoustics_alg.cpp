...
double P_0;
double tau;
double P_r[2000][2000];
double chi[2000][2000];
...
void free_path_loss()
{
	...
	parallel_for(blocked_range<int>(0, nx), [&](const blocked_range<int> &r) {
...
void s_compute_acoustics()
{
	...
	while (step < (int)(MAX_TIME / TIME_STEP))
	{
		...
		{
			parallel_for(blocked_range<int>(0, ny), [&](const blocked_range<int> &r) {
